...
| ------------------ | -------------------------------------- | -------------------------------------------------------------------------------- |
| Auth               | Login, refresh, logout, recuperación   | `/v2/users/login`, `/v2/users/refresh-token`, `/v2/users/logout`                 |
| Users              | Perfil, actualización, eliminación     | `/v2/users/me`, `/v2/users/update`, `/v2/users/remove`                           |
...
| Properties         | Casas, detalle, escaparate, calendario | `/v2/properties/mine`, `/v2/properties/{id}`, `/v2/properties/all`               |
| Surveys            | Definicion, respuestas, resume, status | `/surveys/mobile/{slug}`, `/surveys/mobile/{slug}/responses`                     |
| Invitations        | Aceptar, registrarse y aceptar         | `/v2/invitations/accept/{token}`, `/v2/invitations/register-and-accept`          |
...
| Push Notifications | Registro de token, activación          | `/v2/users/register-device-token`, `/v2/users/enable-notifications/{token}`      |
...
La API v2 identifica cada temporada solo por su nombre localizado, que es texto libre por calendario: algunas casas usan los nombres completos ("Temporada baja") y otras los abreviados de los tags ("T. baja"). `resolveSeasonInfo` resuelve el slug buscando ese nombre (EN/ES) entre los `name`/`name_es` y `tag`/`tag_es` de `SEASONS`; sin coincidencia cae al slug `high`.
