...
<CardGroup cols={3}>
  ...
  <Card title="message-attachments/" icon="paperclip">
    **11 componentes** para renderizado de diferentes tipos de adjuntos: imágenes, videos, archivos,
    notas de voz, ubicaciones, etc. Los PDF se renderizan localmente con pdf.js bundleado
    (`PdfViewerModal`, asset `src/shared/assets/pdf-viewer.html`), sin salir del dispositivo; el
    resto de documentos previsualizables siguen usando Google Docs Viewer vía `WebViewModal`.
    Regenerar el asset con `npm run build:pdf-viewer` tras actualizar `pdfjs-dist`.
  </Card>
