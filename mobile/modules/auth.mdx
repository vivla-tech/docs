...
| -------------- | ------------------------- | ------------------------------ |
| `LoginScreen`  | `/(auth)/login`           | Login con email/contraseña     |
...
| -------- | ---------------------------- | ----------------------------- |
| `POST`   | `/v2/users/login`            | Login con credenciales        |
| `PUT`    | `/v2/users/recover-password` | Solicitar reset de contraseña |
| `DELETE` | `/v2/users/logout`           | Cerrar sesión                 |
| `DELETE` | `/v2/users/remove`           | Eliminar cuenta               |
| `POST`   | `/v2/users/refresh-token`    | Renovar access token          |
| `GET`    | `/v2/users/me`               | Obtener usuario actual        |

<Note>
  `/v2/users/me` devuelve el DTO de usuario de v2 (id numérico, `name`/`lastName` separados, `lang`
  y las listas de casas). `mapUserV2ToUser` lo traduce al modelo `User` de la app, que espera `id`
  string y `displayName`; las listas se descartan porque las casas se leen de `/v2/properties/mine`.
</Note>
