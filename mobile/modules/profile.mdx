...
| `GET`  | `/calendars/available-bookings/{propertyId}` | Estadías disponibles del usuario para una propiedad |
| `POST` | `/v2/users/register-device-token`            | Registrar device token para notificaciones push     |
| `PUT`  | `/v2/users/enable-notifications/{token}`     | Habilitar o deshabilitar notificaciones push        |
| `PUT`  | `/v2/users/update`                           | Actualizar perfil del usuario (nombre, imagen)      |

<Note>
  Las dos rutas de push vivían antes bajo `/v2/notifications`; el módulo se integró en usuarios. El
  body de `/v2/users/update` es el de v2 (`name`, `lastName`, `profileImage`, `lang`), así que el
  `displayName` del modelo de la app se separa en `name` y `lastName` antes de enviarlo.
</Note>
