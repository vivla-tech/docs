...
### Guard de propiedad en el write mobile

Los dos caminos de **escritura** mobile (`submitResponse` y `completeResponse`) validan, tras resolver el scope, que una respuesta con scope de **reserva** solo la pueda escribir el ocupante de esa reserva (`SurveysService.assertBookingScopeOwnedByRespondent`). Antes, con `MobileAuthGuard` como placeholder y el scope guardado tal cual, cualquier deep link con un `bookingId` real podía abrir **y guardar** una respuesta contra una reserva ajena — un propietario acababa rellenando reviews de llegada de casas reasignadas a otros huéspedes.

- Solo se comprueba el scope `booking`; los demás (`property`/`user`/`home`) pasan de largo.
- Como corre **después** de la resolución de scope (VIV-2505), en una semana cedida el scope ya apunta a la fila hija del ocupante, cuyo `user_id` es el propio ocupante. Se acepta además el `occupantUid` embebido en el `raw_data` de la oferta por si el redirect no pudo dispararse, así un ocupante legítimo nunca se bloquea de más.
- Un `scopeId` sin fila en `bookings_snapshot` mantiene el pass-through de hoy (se guarda tal cual). Cualquier otro caso responde `403` (`You can only respond to a survey for your own booking`); los caminos de solo lectura (`GET`/`resume`/`getScopeStatuses`) no se ven afectados.
...
<Warning>
**Una fila por identidad — no uses `.upsert()` aquí.** La identidad de una fila
abarca `property_id` y `user_id`, y toda escritura deja uno de los dos en NULL
(un recálculo por casa no tiene usuario, y al revés). En Postgres un NULL nunca
es igual a otro NULL, así que el `UNIQUE` original nunca casaba: cada recálculo
**insertaba** una fila en vez de actualizarla. Producción llegó a 242 filas
duplicadas de 435, con una casa acumulando diez copias de la misma métrica.

La migración 229 limpió los duplicados y cambió el `UNIQUE` por un índice con
`COALESCE` que sí trata esos NULL como iguales. Ambos escritores
(`ScoreSummaryService` y el CLI `sync-firebase-scores`) pasan ahora por
`writeScoreSummary()` (`score-summary-write.util.ts`), que localiza la fila con
`.is()` — la única forma de casar un NULL — y la actualiza.

Si lees esta tabla a mano, ordena por `last_computed_at`: puede haber
duplicados anteriores a la migración en entornos que aún no la hayan aplicado.
</Warning>
