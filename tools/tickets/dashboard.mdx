...
<Note>
  Los filtros derivados dependen de funciones puras (`resolveTicketType`, `computeSlaStatus`, `isDormant`) que no se pueden traducir a SQL sin duplicar las reglas. Desde VIV-2766 el listado **siempre** trae el conjunto filtrado por columnas, deriva en memoria y pagina localmente — barato para el working set de CX (miles, no decenas de miles): el orden «pendientes de comentario primero» tiene que ser **global**, no solo dentro de la página, así que ya no queda un camino de paginación en Postgres (`.range`). `pendingCost` sigue siendo SQL-pushable: al ser una columna booleana se aplica en la query (`.eq('cost_pending', true)`) y compone por AND con el resto.
</Note>

### Etiqueta «Pendiente de comentario»

Un ticket **abierto** —solo estados `created` / `assigned` / `in_progress` (`PENDING_COMMENT_OPEN_STATUSES`, no el `isOpenStatus` general, que incluye `improvement_proposal`)— sin actividad durante **48 h** (`PENDING_COMMENT_THRESHOLD_HOURS`) se marca con el derivado `pendingComment` y luce un **chip ámbar** en la lista, con el estilo del chip de SLA / coste pendiente (VIV-2766).

- **Última actividad** (`computeLastActivityAt`) = lo más reciente entre el último comentario o nota interna (`chat_ticket_comments`, sin filtrar `is_public`) y el último cambio de estado (`chat_ticket_status_history`), con `created_at` como suelo. No usa `updated_at` ni el denormalizado `last_comment_at`.
- Solo se enriquecen los tickets abiertos (dos queries batch por `ticket_id`); un fallo degrada a «sin actividad» en vez de romper el listado.
- La lista antepone **los pendientes** de forma global y, dentro de cada grupo, conserva `created_at` desc. Es una etiqueta, no un filtro: no reordena en cliente.
