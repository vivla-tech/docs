...
| `booking.stay_review_due` | `daily_stay_review` 10:00 | check-out de hoy y de los 3 días anteriores — lanza stay_review survey (también nudge de salida). La ventana recupera las reservas que CX marca como disfrutadas después de la hora del cron; la clave de idempotencia, que no lleva fecha, garantiza un único envío por reserva |
...
exclusiones (cancelada, archivada, `@vivla.`, propiedad inactiva, y **reserva fantasma** —
`bookings_snapshot.lifecycle != 'present'`) los resuelve
`GET /surveys/reports/survey-candidates?kind=followup|buen_viaje`. Flows sembrados
`is_active=false`.

<Note>
  El guard de reserva fantasma aplica a **todos** los `kind` del endpoint (`arrival`, `stay`,
  `survey_reminder_3d`, `followup`, `buen_viaje`). Una reserva que desaparece del feed origen
  deja su `status` congelado en el último valor (a menudo `active`/`past`), así que el estado
  por sí solo no distingue una reserva viva de una reasignada; `lifecycle` sí
  (`superseded`/`vanished`/`released`/`returned` marcan las muertas). Es `NOT NULL DEFAULT
  'present'` (migración 211), por lo que un `null` solo aparece en filas previas a la columna y
  se trata como presente. Sin esto, el cron saludaría al propietario por una semana ya cedida a
  otro huésped.
</Note>
