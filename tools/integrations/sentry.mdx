...
**Slugs centinela.** Un 404 de `/api/web/` cuyo ultimo segmento es `null` o `undefined` (por mayus/minus) **no** se reporta: es un llamante que serializo un valor nulo en la URL (`String(null)`), un bug del puente aguas arriba y no un enlace publico muerto — no hay 301 que ponerle. El cliente sigue recibiendo el 404 normal; solo se evita que entierre los slugs rotos de verdad.
</Note>
