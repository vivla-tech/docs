...
## Web — Properties (público, v1)

Feed plano de casas listadas en la web, consumido por servicios backend (vivla-api, vivla-mobile, Windmill) que antes tiraban a Webflow. Sigue 100% las convenciones v1: envelope `{ object: 'list', data, has_more, next_cursor }`, items con `object: 'property'`, paginación cursor, snake_case.

Detalle completo (shape, ejemplos, errores, mapeo Webflow → v1) en [Web — Properties (público)](./properties).

| Método | Endpoint | Auth | Descripción |
|--------|----------|------|-------------|
| GET | `/v1/web/properties` | API Key / Bearer (tool-web viewer) | Lista cursor-paginada. Filtros: `type=beach\|ski`, `status=listed,coming_soon,sold_out` (CSV, default `listed`), `location_id`, `limit`, `cursor`. |
| GET | `/v1/web/properties/:slug-o-id` | API Key / Bearer (tool-web viewer) | Detalle por slug público o UUID (`properties.id`). Sirve cualquier estado salvo `hidden`; 404 si no existe o está `hidden`. |
