...
- **`gallery`** trae todas las fotos con `is_visible_on_web=true`, ordenadas por `sort_order`. El `section` permite agrupar client-side ("hero", "studio", etc.). Es un **enum cerrado**: las secciones internas que añadió la migración 256 para las pestañas de la ficha Web 2026 se pliegan sobre las documentadas antes de salir al wire — `interior` y `exterior` → `gallery`, `surroundings` → `location`, `floor_plan` → `other` — así que el vocabulario de este campo no cambia.
