...
<Note>
  ...
  **Ojo con la columna del match.** v2 renombró `homes.old_id` a `homes.external_id` y la pasó de `varchar` a `integer` (en `stage` desde el 2-sep-2026; en producción desde el 15-sep-2026). El provider resuelve en caliente cuál de las dos existe, así que funciona en los dos entornos sin coordinar despliegues. El `integer` pierde los ceros a la izquierda de cuatro casas (`00292439` de Casa Taula II se queda en `292439`), por eso el emparejamiento compara los hid sin ceros a la izquierda. Lo que se guarda en `properties.hid` no cambia: sigue con su cero. **El job incremental de 30 min (`fetchNewOwnerUsers`) usa el mismo resolver:** si el SQL sigue pidiendo `h.old_id`, cada pasada muere con `column h.old_id does not exist` y no avanza el cursor. No se degrada a lista vacía: eso perdería propietarios.

  **`homes.test_home` ya no existe en v2** (borrada en `stage` el 22-jul-2026). Cuando la columna falta, v2 no opina sobre qué casa es de prueba y se respeta el `is_test_home` que viene de Firestore. Si diéramos por hecho que es `false`, una casa de prueba podría acabar activa en tools.
