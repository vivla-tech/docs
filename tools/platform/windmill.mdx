...
<Note>
  ...
  **Ojo con la columna del match.** v2 renombró `homes.old_id` a `homes.external_id` y la pasó de `varchar` a `integer` (en `stage` desde el 2-sep-2026; producción todavía tiene `old_id`). El provider resuelve en caliente cuál de las dos existe, así que funciona en los dos entornos sin coordinar despliegues. El `integer` pierde los ceros a la izquierda de cuatro casas (`00292439` de Casa Taula II se queda en `292439`), por eso el emparejamiento compara los hid sin ceros a la izquierda. Lo que se guarda en `properties.hid` no cambia: sigue con su cero.
